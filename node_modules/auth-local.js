...
// services will respond with a common error message:
    authenticate( req, res, next, function () {
        ...
        AD.Comm.Service.sendError( req, res, {errorID:55, errorMSG:authLabels['[site.autherrors.Reauthenticate]'][req.aRAD.defaults['siteDefaultLanguage']]} );
    });
...
//  if not, then create an unauthenticed viewer

    var msg = '   - authentication.authenticate() ';
    var currentViewer = AD.Viewer.trueViewer(req);
    var path = url.parse(req.url);
    
    // if a .viewer has been added to session already, 
    // keep on.
...
//log(req,'current session:');
//log(req,req.session);

    // if (viewer.isValidLogin(userID, pWord)) {
...
logDump(req);        
            // handle error:
            // return json Failure
            AD.Comm.Service.sendError( req, res, {errorID:55, errorMSG:authLabels['[site.autherrors.InvalidUP]'][req.aRAD.defaults['siteDefaultLanguage']]} );
